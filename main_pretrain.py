...
class MyLoad(object):
    ...
    def __call__(self, img):
        ...
        """
        #x = PIL.Image.open("converted_bird96.png")
        #print("Loaded tensor:",x.shape)
        x = np.load("test.npy").squeeze(axis=0)
...
def myloader(file_name):
    ...
    resp = req.get(url)
    #print(resp.text)
    x = np.load(resp.text).squeeze(axis=0)
    x = torch.from_numpy(x)
    os.remove(resp.text)
    #print("Loaded tensor:",x.shape)
    return x 
...
def my_is_valid_file(file_name):
    ...
    ret_val =  True if extension == "JPEG" else False
    #if (ret_val == True):
    #    print("in my is valid file:",file_name,ret_val)
    return ret_val
...
def main(args):
    ...
    #       my_trans_obj
    #       ])
    #dataset_train = datasets.ImageFolder(os.path.join(args.data_path, 'train'), transform=transform_train,is_valid_file = my_is_valid_file)
    #dataset_train = datasets.ImageFolder(os.path.join(args.data_path, 'train'), transform=transform_train, loader = myloader,is_valid_file = my_is_valid_file)
